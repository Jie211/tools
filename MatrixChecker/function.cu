...
void gethead_jad(const char *file1, const char *file2, const char *file3, const char *file4, int *N, int *NZ)
{
  ...
  if((in4 = fopen(file4, "r")) == NULL)
  {
    printf("%s file open error", file4);
    exit(0);
  }
  ...
  if(N11 != N21 || N21!=N31 || N31!=N41 || N41!=N11)
  {
    printf("N was not same in 4files\n");
    exit(0);
  }

  if(NZ1 != NZ2 || NZ2!=NZ3 || NZ3!=NZ4 || NZ4!=NZ1)
  {
    printf("NZ was not same in 4files\n");
    exit(0);
  }
  ...
  *NZ = NZ1;
  ...
  printf("NZ1=%d NZ2=%d NZ3=%d NZ4=%d\n",NZ1,NZ2,NZ3,NZ4);
#endif
...
void getmaxnz_jad(const char *file1, int *MAXNZ)
{
  FILE *in;
  int hoge1, hoge2, hoge3;
  int maxnz;
...
void getdata_jad(const char *file1, const char *file2, const char *file3,const char *file4, int *col,int *perm,int *nz, int *jd, double *val,double *b,double *x, int N, int NZ, int MAXNZ)
{
  FILE *in1,*in2,*in3,*in4;
