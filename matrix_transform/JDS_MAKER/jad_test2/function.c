...
void getdata(const char *file1, const char *file2, const char *file3,int *ptr,mat *matrix,double *b,double *x, int N, int NZ)
{
  FILE *in1,*in2,*in3;
  ...
  for(int i=0;i<NZ;i++)
  {
    fscanf(in1,"%d %d %le\n",&getint, &getint2,&getdouble);
    matrix[i].row = getint;
    matrix[i].col = getint2;
    matrix[i].val = getdouble;
    matrix[i].nz  = 0;
    matrix[i].perm= 0;
    matrix[i].col_n = 0;
    matrix[i].row_n = 0;
